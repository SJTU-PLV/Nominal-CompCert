...
Require Import Smallstep CallconvNew.
...
Section FSIM.
    ...
    match_contframes_intro i s1 s2:
      ...
      (forall r1 r2 s1' gw', (get wk) o-> gw' /\ GS.match_reply cc (set wk gw') r1 r2 ->
       ...
       exists idx s2',
         ...
         exists gw'', gw' *-> gw'' /\ GS.fsim_match_states (HL i) se1 se2 wk' gw'' idx s1' s2') ->
      match_contframes wk wk' 
        (st L1 i s1)
        (st L2 i s2).

  Inductive match_states: GS.gworld cc -> index -> list (frame L1) -> list (frame L2) -> Prop :=
    | match_states_cons gw wk idx f1 f2 k1 k2:
  ...
  Lemma step_simulation:
    ...
    exists idx' s2',
      ...
      exists gw', gw*-> gw' /\ match_states gw' idx' s1' s2'.
  Proof.
    ...
    - (* internal step *)
      ...
      * destruct Hs2'; [left | right]; intuition eauto using star_internal, plus_internal.
        constructor. auto.
      * destruct Hs' as [gw' [ACC Hs']].
        exists gw'. split. auto. econstructor; eauto.
        econstructor; eauto. 
    ...
    - (* cross-component return *)
      ...
      pose proof (GS.fsim_lts (HL i) _ _ H3 H8).
      edestruct @GS.fsim_match_final_states as (r2 & Hr2 & ACC & Hr); eauto.
      inv H7. inv H6; subst_dep. edestruct H9 as (idx' & s2' & Hs2'& gw'' & ACC2 & Hs'); eauto.
      eexists (existT _ j idx'), _. split.
  ...
  Lemma final_states_simulation:
    ...
    exists r2, final_state L2 se2 s2 r2 /\ (get w) o-> wp /\ GS.match_reply cc (set w wp) r1 r2.
  Proof.
    clear. intros wp idx s1 s2 r1 Hs Hr1. destruct Hr1 as [i s1 r1 Hr1].
    inv Hs. inv H5. inv H3. subst_dep. clear idx0.
    pose proof (GS.fsim_lts (HL i) _ _ H1 H5).
    edestruct @GS.fsim_match_final_states as (r2 & Hr2 & ACC & Hr); eauto.
  ...
  Lemma external_simulation:
    ...
    exists idx' s2', after_external L2 se2 s2 rx2 s2' /\ exists wp''', wp'' *-> wp''' /\ match_states wp''' idx' s1' s2'.
  Proof.
    clear - HL Hse1.
    intros wp idx s1 s2 q1 Hs Hq1. destruct Hq1 as [i s1 qx1 k1 Hqx1 Hvld].
    inv Hs. inv H3. subst_dep. clear idx0.
    pose proof (GS.fsim_lts (HL i) _ _ H1 H6) as Hi.
    ...
    + constructor. eauto.
      ...
      erewrite GS.fsim_match_valid_query; eauto.
    + inv H3; subst_dep.
      edestruct H as (idx' & s2' & Hs2' & gw''' & ACC1 & Hs); eauto.
