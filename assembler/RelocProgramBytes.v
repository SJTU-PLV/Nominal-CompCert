...
Definition section := RelocProg.section byte byte.

Definition sectable := RelocProg.sectable byte byte.
...
Definition sec_size (s: section) : Z :=
  ...
  | sec_text lb
  | sec_rwdata lb
  | sec_rodata lb => Z.of_nat (length lb)
  end.

Definition sections_size stbl :=
  fold_left (fun sz sec => sz + (sec_size sec)) stbl 0.

Definition program := RelocProg.program fundef unit byte byte.
