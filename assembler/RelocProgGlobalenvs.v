...
(** Global environments for RelocProg which parameterizes the data type *)
...
(* Since we use a new global environment, we need to redefine the storing of the init data *)
...
(* Allocation of external and common symbols *)
...
(** globals_initialized, which is similar to the 'globals_initialized' in Globalenv.v. It is used to establish the relation between the programs and their initialized memory  *)
...
(* The memory initialization: first allocate the sections and then allocate the external and common symbols *)
...
(** Start to prove the initialization of memory confirms to the global_initialized: lemma called init_mem_characterization_gen  *)
...
(** Some properties of initialization of data  *)
