...
(** * See InjectComp1.v, we do not need more memory operations than in InjectFootprint.v. *)

(** i.e. this construction is *the same* as injp @ injp ≡ injp. we do not need to write an extra file or construction,
 we only need to release the hypothesis from match_stbls j12 se1 se2 to match_stbls' j12 se1 se2. *)
