...
Definition write_B (REX:bits)  (B_value:bits) : bits :=
	B_value++(REX>@[1]).

Definition read_B (REX:bits) : bits :=
	REX~@[1].

Definition write_R (REX:bits)  (R_value:bits) : bits :=
	(REX~@[2])++R_value++(REX>@[3]).

Definition read_R (REX:bits) : bits :=
	(REX>@[2])~@[1].

Definition write_W (REX:bits)  (W_value:bits) : bits :=
	(REX~@[3])++W_value++(REX>@[4]).

Definition read_W (REX:bits) : bits :=
	(REX>@[3])~@[1].

Definition write_X (REX:bits)  (X_value:bits) : bits :=
	(REX~@[1])++X_value++(REX>@[2]).

Definition read_X (REX:bits) : bits :=
	(REX>@[1])~@[1].

Definition write_base (SIB:bits)  (base_value:bits) : bits :=
	base_value++(SIB>@[3]).

Definition read_base (SIB:bits) : bits :=
	SIB~@[3].

Definition write_cccode (condi:bits)  (cccode_value:bits) : bits :=
	cccode_value++(condi>@[4]).

Definition read_cccode (condi:bits) : bits :=
	condi~@[4].

Definition write_col (Opcode:bits)  (col_value:bits) : bits :=
	col_value++(Opcode>@[3]).

Definition read_col (Opcode:bits) : bits :=
	Opcode~@[3].

Definition write_cprefix (condi:bits)  (cprefix_value:bits) : bits :=
	(condi~@[4])++cprefix_value.

Definition read_cprefix (condi:bits) : bits :=
	(condi>@[4]).

Definition write_index (SIB:bits)  (index_value:bits) : bits :=
	(SIB~@[3])++index_value++(SIB>@[6]).

Definition read_index (SIB:bits) : bits :=
	(SIB>@[3])~@[3].

Definition write_mod (ModRM:bits)  (mod_value:bits) : bits :=
	(ModRM~@[6])++mod_value.

Definition read_mod (ModRM:bits) : bits :=
	(ModRM>@[6]).

Definition write_page (Opcode:bits)  (page_value:bits) : bits :=
	(Opcode~@[3])++page_value++(Opcode>@[4]).

Definition read_page (Opcode:bits) : bits :=
	(Opcode>@[3])~@[1].

Definition write_reg_op (ModRM:bits)  (reg_op_value:bits) : bits :=
	(ModRM~@[3])++reg_op_value++(ModRM>@[6]).

Definition read_reg_op (ModRM:bits) : bits :=
	(ModRM>@[3])~@[3].

Definition write_rm (ModRM:bits)  (rm_value:bits) : bits :=
	rm_value++(ModRM>@[3]).

Definition read_rm (ModRM:bits) : bits :=
	ModRM~@[3].

Definition write_rmagic (REX:bits)  (rmagic_value:bits) : bits :=
	(REX~@[4])++rmagic_value.

Definition read_rmagic (REX:bits) : bits :=
	(REX>@[4]).

Definition write_row (Opcode:bits)  (row_value:bits) : bits :=
	(Opcode~@[4])++row_value.

Definition read_row (Opcode:bits) : bits :=
	(Opcode>@[4]).

Definition write_scale (SIB:bits)  (scale_value:bits) : bits :=
	(SIB~@[6])++scale_value.

Definition read_scale (SIB:bits) : bits :=
	(SIB>@[6]).
...
Definition encode_AddrE element  inBits  :=
	match element with
| AddrE11 uvar32_0 => let bits00 := inBits in
let bits01 := write_mod bits00 [false;false] in
let bits02 := write_rm bits01 [true;false;true] in
let result0 := bits02 in
let bits11 := (proj1_sig uvar32_0) in
let result1 := bits11 in
OK (result0 ++ result1)
| AddrE9 uvar2_0 uvar3_1 uvar32_2 => let bits00 := inBits in
let bits01 := write_mod bits00 [false;false] in
let bits02 := write_rm bits01 [false;false;true] in
let result0 := bits02 in
let bits10 := (repeat false 8) in
let bits11 := write_base bits10 [true;false;true] in
let bits12 := write_scale bits11 (proj1_sig uvar2_0) in
let bits13 := write_index bits12 (proj1_sig uvar3_1) in
let result1 := bits13 in
let bits21 := (proj1_sig uvar32_2) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| AddrE5 uvar2_0 uvar3_1 uvar3_2 uvar32_3 => let bits00 := inBits in
let bits01 := write_mod bits00 [false;true] in
let bits02 := write_rm bits01 [false;false;true] in
let result0 := bits02 in
let bits10 := (repeat false 8) in
let bits11 := write_scale bits10 (proj1_sig uvar2_0) in
let bits12 := write_index bits11 (proj1_sig uvar3_1) in
let bits13 := write_base bits12 (proj1_sig uvar3_2) in
let result1 := bits13 in
let bits21 := (proj1_sig uvar32_3) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| AddrE0 uvar3_0 => let bits00 := inBits in
let bits01 := write_mod bits00 [true;true] in
let bits02 := write_rm bits01 (proj1_sig uvar3_0) in
let result0 := bits02 in
OK (result0)
end.

Program Definition decode_AddrE bin : res (AddrE*nat) :=
	if AddrE11_bp bin then
let code0 := bin in
do code1 <- try_skip_n code0 8;
do token0 <- try_first_n code1 32;
let uvar32_0 := token0 in
if assertLength uvar32_0 32 then
do code2 <- try_skip_n code1 32;
OK ((AddrE11 (uvar32_0)), 40)
...
let code0 := bin in
do code1 <- try_skip_n code0 8;
do token0 <- try_first_n code1 8;
let uvar2_0 := read_scale token0 in
if assertLength uvar2_0 2 then
let uvar3_1 := read_index token0 in
if assertLength uvar3_1 3 then
do code2 <- try_skip_n code1 8;
do token1 <- try_first_n code2 32;
let uvar32_2 := token1 in
if assertLength uvar32_2 32 then
do code3 <- try_skip_n code2 32;
OK ((AddrE9 (uvar2_0) (uvar3_1) (uvar32_2)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else Error(msg"impossible")
else
...
let uvar3_1 := read_index token0 in
if assertLength uvar3_1 3 then
let uvar3_2 := read_base token0 in
if assertLength uvar3_2 3 then
do code2 <- try_skip_n code1 8;
do token1 <- try_first_n code2 32;
let uvar32_3 := token1 in
if assertLength uvar32_3 32 then
do code3 <- try_skip_n code2 32;
OK ((AddrE5 (uvar2_0) (uvar3_1) (uvar3_2) (uvar32_3)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else Error(msg"impossible")
else Error(msg"impossible")
...
let code0 := bin in
do token0 <- try_first_n code0 8;
let uvar3_0 := read_rm token0 in
...
Definition encode_Instruction element  :=
	match element with
| REX_WRXB uvar1_0 uvar1_1 uvar1_2 uvar1_3 => let bits00 := (repeat false 8) in
let bits01 := write_rmagic bits00 [false;false;true;false] in
let bits02 := write_W bits01 (proj1_sig uvar1_0) in
let bits03 := write_R bits02 (proj1_sig uvar1_1) in
let bits04 := write_X bits03 (proj1_sig uvar1_2) in
let bits05 := write_B bits04 (proj1_sig uvar1_3) in
let result0 := bits05 in
OK (result0)
| Override => let bits01 := [false;true;true;false;false;true;true;false] in
let result0 := bits01 in
OK (result0)
| REP => let bits01 := [true;true;false;false;true;true;true;true] in
let result0 := bits01 in
OK (result0)
| REPNZ => let bits01 := [false;true;false;false;true;true;true;true] in
let result0 := bits01 in
OK (result0)
| Psubl_ri uvar3_0 uvar32_1 => let bits01 := [true;false;false;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;false;true] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
let bits21 := (proj1_sig uvar32_1) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pbsqrtsd uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;false;false;false;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
let bits23 := write_rm bits22 (proj1_sig uvar3_1) in
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Psbbl_rr uvar3_0 uvar3_1 => let bits01 := [true;false;false;true;true;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 (proj1_sig uvar3_0) in
let bits13 := write_rm bits12 (proj1_sig uvar3_1) in
let result1 := bits13 in
OK (result0 ++ result1)
| Prep_movsl => let bits01 := [true;false;true;false;false;true;false;true] in
let result0 := bits01 in
OK (result0)
| Pmovsq_rm AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;true;true;true;true;true;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pmovsq_mr AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;true;true;false;true;false;true;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pminsd uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;false;true;true;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pmaxsd uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;true;true;true;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pbswap32 uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;false;false] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
OK (result0 ++ result1)
| Pbsrl uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;false;true;true;true;true;false;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pbsfl uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;false;true;true;true;true;false;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Paddl_mi AddrE uvar32_1 => let bits01 := [false;false;false;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 [false;false;false] in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
let bits21 := (proj1_sig uvar32_1) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Paddl_GvEv AddrE uvar3_0 => let bits01 := [true;true;false;false;false;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Paddl_EvGv AddrE uvar3_0 => let bits01 := [true;false;false;false;false;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Padcl_rr uvar3_0 uvar3_1 => let bits01 := [true;false;false;false;true;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 (proj1_sig uvar3_0) in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Padcl_ri uvar3_0 uvar8_1 => let bits01 := [true;true;false;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;true;false] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
let bits21 := (proj1_sig uvar8_1) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pjcc_rel uvar4_0 uvar32_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_cprefix bits10 [false;false;false;true] in
let bits12 := write_cccode bits11 (proj1_sig uvar4_0) in
let result1 := bits12 in
let bits21 := (proj1_sig uvar32_1) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pret_iw uvar16_0 => let bits01 := [false;true;false;false;false;false;true;true] in
let result0 := bits01 in
let bits11 := (proj1_sig uvar16_0) in
let result1 := bits11 in
OK (result0 ++ result1)
| Pret => let bits01 := [true;true;false;false;false;false;true;true] in
let result0 := bits01 in
OK (result0)
| Pcall_r uvar3_0 => let bits01 := [true;true;true;true;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;true;false] in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Pcall_ofs uvar32_0 => let bits01 := [false;false;false;true;false;true;true;true] in
let result0 := bits01 in
let bits11 := (proj1_sig uvar32_0) in
let result1 := bits11 in
OK (result0 ++ result1)
| Pnop => let bits01 := [false;false;false;false;true;false;false;true] in
let result0 := bits01 in
OK (result0)
| Pjmp_Ev AddrE => let bits01 := [true;true;true;true;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 [false;false;true] in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pjmp_l_rel uvar32_0 => let bits01 := [true;false;false;true;false;true;true;true] in
let result0 := bits01 in
let bits11 := (proj1_sig uvar32_0) in
let result1 := bits11 in
OK (result0 ++ result1)
| Pxorps_d_GvEv AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;true;true;false;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pcomiss_d_ff uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;true;true;true;false;true;false;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pdivss_d_ff uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;true;true;true;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pmuls_d_ff uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;false;false;true;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Psubs_d_ff uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;false;true;true;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pandps_d_fm AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;false;true;false;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Padds_d_ff uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;false;false;true;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Psetcc uvar4_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_cprefix bits10 [true;false;false;true] in
let bits12 := write_cccode bits11 (proj1_sig uvar4_0) in
let result1 := bits12 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 [false;false;false] in
let bits23 := write_rm bits22 (proj1_sig uvar3_1) in
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pcmov uvar4_0 uvar3_1 uvar3_2 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_cprefix bits10 [false;false;true;false] in
let bits12 := write_cccode bits11 (proj1_sig uvar4_0) in
let result1 := bits12 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_1) in
let bits23 := write_rm bits22 (proj1_sig uvar3_2) in
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Ptestl_EvGv AddrE uvar3_0 => let bits01 := [true;false;true;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Ptestl_ri uvar3_0 uvar32_1 => let bits01 := [true;true;true;false;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;false;false] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
let bits21 := (proj1_sig uvar32_1) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pcmpl_ri uvar3_0 uvar32_1 => let bits01 := [true;false;false;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;true;true] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
let bits21 := (proj1_sig uvar32_1) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pcmpl_GvEv AddrE uvar3_0 => let bits01 := [true;true;false;true;true;true;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pcmpl_EvGv AddrE uvar3_0 => let bits01 := [true;false;false;true;true;true;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Prorl_ri uvar3_0 uvar8_1 => let bits01 := [true;false;false;false;false;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 [true;false;false] in
let bits12 := write_mod bits11 [true;true] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
let bits21 := (proj1_sig uvar8_1) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Prolw_ri uvar3_0 uvar8_1 => let bits01 := [true;false;false;false;false;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;false;false] in
...
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pshld_ri uvar3_0 uvar3_1 uvar8_2 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;false;true;false;false;true;false;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
let bits23 := write_rm bits22 (proj1_sig uvar3_1) in
let result2 := bits23 in
let bits31 := (proj1_sig uvar8_2) in
let result3 := bits31 in
OK (result0 ++ result1 ++ result2 ++ result3)
| Psarl_rcl uvar3_0 => let bits01 := [true;true;false;false;true;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;true;true] in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Psarl_ri uvar3_0 uvar8_1 => let bits01 := [true;false;false;false;false;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;true;true] in
...
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pshrl_rcl uvar3_0 => let bits01 := [true;true;false;false;true;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;false;true] in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Pshrl_ri uvar3_0 uvar8_1 => let bits01 := [true;false;false;false;false;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;false;true] in
...
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Psall_rcl uvar3_0 => let bits01 := [true;true;false;false;true;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;false;true] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
OK (result0 ++ result1)
| Psall_ri uvar3_0 uvar8_1 => let bits01 := [true;false;false;false;false;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;false;true] in
...
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pnotl uvar3_0 => let bits01 := [true;true;true;false;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;true;false] in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Pxorl_GvEv AddrE uvar3_0 => let bits01 := [true;true;false;false;true;true;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pxorl_EvGv AddrE uvar3_0 => let bits01 := [true;false;false;false;true;true;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pxorl_ri uvar3_0 uvar32_1 => let bits01 := [true;false;false;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;true;true] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
let bits21 := (proj1_sig uvar32_1) in
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Porl_GvEv AddrE uvar3_0 => let bits01 := [true;true;false;true;false;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Porl_EvGv AddrE uvar3_0 => let bits01 := [true;false;false;true;false;false;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Porl_ri uvar3_0 uvar32_1 => let bits01 := [true;false;false;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;false;false] in
...
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pandl_ri uvar3_0 uvar32_1 => let bits01 := [true;false;false;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;false;true] in
...
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pandl_GvEv AddrE uvar3_0 => let bits01 := [true;true;false;false;false;true;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pandl_EvGv AddrE uvar3_0 => let bits01 := [true;false;false;false;false;true;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pidivl_r uvar3_0 => let bits01 := [true;true;true;false;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;true;true] in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Pdivl_r uvar3_0 => let bits01 := [true;true;true;false;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;true;true] in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Pcltd => let bits01 := [true;false;false;true;true;false;false;true] in
let result0 := bits01 in
OK (result0)
| Pmull_r uvar3_0 => let bits01 := [true;true;true;false;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;false;true] in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Pimull_r uvar3_0 => let bits01 := [true;true;true;false;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;false;true] in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Pimull_ri uvar3_0 uvar3_1 uvar32_2 => let bits01 := [true;false;false;true;false;true;true;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 (proj1_sig uvar3_0) in
...
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pimull_GvEv AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;true;true;true;false;true;false;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Psubl_GvEv AddrE uvar3_0 => let bits01 := [true;true;false;true;false;true;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Psubl_EvGv AddrE uvar3_0 => let bits01 := [true;false;false;true;false;true;false;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Paddl_ri uvar3_0 uvar32_1 => let bits01 := [true;false;false;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [false;false;false] in
...
let result2 := bits21 in
OK (result0 ++ result1 ++ result2)
| Pnegl uvar3_0 => let bits01 := [true;true;true;false;true;true;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 [true;true;false] in
let bits13 := write_rm bits12 (proj1_sig uvar3_0) in
let result1 := bits13 in
OK (result0 ++ result1)
| Pleal AddrE uvar3_0 => let bits01 := [true;false;true;true;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pcvtsi2ss_d_fr uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;true;false;true;false;true;false;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pcvttss_d_2si_rf uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;false;true;true;false;true;false;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pcvtsd2ss_d_ff uvar3_0 uvar3_1 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;true;false;true;true;false;true;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_mod bits20 [true;true] in
let bits22 := write_reg_op bits21 (proj1_sig uvar3_0) in
...
let result2 := bits23 in
OK (result0 ++ result1 ++ result2)
| Pmovsxd_GvEv AddrE uvar3_0 => let bits01 := [true;true;false;false;false;true;true;false] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pmovsw_GvEv AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;true;true;true;true;true;false;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pmovzw_GvEv AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;true;true;false;true;true;false;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pmovsb_GvEv AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;true;true;true;true;true;false;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pmovzb_rm AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;true;true;false;true;true;false;true] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pmovb_rm AddrE uvar3_0 => let bits01 := [false;true;false;true;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pmovb_mr AddrE uvar3_0 => let bits01 := [false;false;false;true;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pxchg_rr uvar3_0 uvar3_1 => let bits01 := [true;true;true;false;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_mod bits10 [true;true] in
let bits12 := write_reg_op bits11 (proj1_sig uvar3_0) in
...
let result1 := bits13 in
OK (result0 ++ result1)
| Pflds_m AddrE => let bits01 := [true;false;false;true;true;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 [false;false;false] in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pfstps_m AddrE => let bits01 := [true;false;false;true;true;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 [true;true;false] in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pfstpl_m AddrE => let bits01 := [true;false;true;true;true;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 [true;true;false] in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pfldl_m AddrE => let bits01 := [true;false;true;true;true;false;true;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 [false;false;false] in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pmovss_d_fm AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [false;false;false;false;true;false;false;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pmovss_d_mf AddrE uvar3_0 => let bits01 := [true;true;true;true;false;false;false;false] in
let result0 := bits01 in
let bits11 := [true;false;false;false;true;false;false;false] in
let result1 := bits11 in
let bits20 := (repeat false 8) in
let bits21 := write_reg_op bits20 (proj1_sig uvar3_0) in
do bits22 <- encode_AddrE AddrE bits21;
let result2 := bits22 in
OK (result0 ++ result1 ++ result2)
| Pmovl_rm AddrE uvar3_0 => let bits01 := [true;true;false;true;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pmovl_mr AddrE uvar3_0 => let bits01 := [true;false;false;true;false;false;false;true] in
let result0 := bits01 in
let bits10 := (repeat false 8) in
let bits11 := write_reg_op bits10 (proj1_sig uvar3_0) in
do bits12 <- encode_AddrE AddrE bits11;
let result1 := bits12 in
OK (result0 ++ result1)
| Pmovl_ri uvar3_0 uvar32_1 => let bits00 := (repeat false 8) in
let bits01 := write_row bits00 [true;true;false;true] in
let bits02 := write_page bits01 [true] in
let bits03 := write_col bits02 (proj1_sig uvar3_0) in
let result0 := bits03 in
let bits11 := (proj1_sig uvar32_1) in
let result1 := bits11 in
OK (result0 ++ result1)
end.
...
let code0 := bin in
do token0 <- try_first_n code0 8;
let uvar1_0 := read_W token0 in
if assertLength uvar1_0 1 then
let uvar1_1 := read_R token0 in
if assertLength uvar1_1 1 then
let uvar1_2 := read_X token0 in
if assertLength uvar1_2 1 then
let uvar1_3 := read_B token0 in
if assertLength uvar1_3 1 then
do code1 <- try_skip_n code0 8;
OK ((REX_WRXB (uvar1_0) (uvar1_1) (uvar1_2) (uvar1_3)), 8)
else Error(msg"impossible")
else Error(msg"impossible")
else Error(msg"impossible")
else Error(msg"impossible")
...
do code2 <- try_skip_n code1 8;
do token1 <- try_first_n code2 32;
let uvar32_1 := token1 in
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Psubl_ri (uvar3_0) (uvar32_1)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do code1 <- try_skip_n code0 8;
do code2 <- try_skip_n code1 8;
do token0 <- try_first_n code2 8;
let uvar3_0 := read_reg_op token0 in
if assertLength uvar3_0 3 then
let uvar3_1 := read_rm token0 in
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Pbsqrtsd (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code2 <- try_skip_n code1 8;
OK ((Psbbl_rr (uvar3_0) (uvar3_1)), 16)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
let uvar3_0 := read_reg_op token0 in
if assertLength uvar3_0 3 then
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pmovsq_rm AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pmovsq_mr AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
let code0 := bin in
do code1 <- try_skip_n code0 8;
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
do token0 <- try_first_n code2 32;
let uvar32_1 := token0 in
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Paddl_mi AddrE (uvar32_1)), 40 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Paddl_GvEv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Paddl_EvGv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code2 <- try_skip_n code1 8;
OK ((Padcl_rr (uvar3_0) (uvar3_1)), 16)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar3_0 3 then
do code2 <- try_skip_n code1 8;
do token1 <- try_first_n code2 8;
let uvar8_1 := token1 in
if assertLength uvar8_1 8 then
do code3 <- try_skip_n code2 8;
OK ((Padcl_ri (uvar3_0) (uvar8_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do code1 <- try_skip_n code0 8;
do token0 <- try_first_n code1 8;
let uvar4_0 := read_cccode token0 in
...
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Pjcc_rel (uvar4_0) (uvar32_1)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
let code0 := bin in
do code1 <- try_skip_n code0 8;
do token0 <- try_first_n code1 16;
let uvar16_0 := token0 in
if assertLength uvar16_0 16 then
do code2 <- try_skip_n code1 16;
OK ((Pret_iw (uvar16_0)), 24)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pjmp_Ev AddrE), 8 + localLength0)
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pxorps_d_GvEv AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Pcomiss_d_ff (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Pdivss_d_ff (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Pmuls_d_ff (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Psubs_d_ff (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pandps_d_fm AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Padds_d_ff (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do code2 <- try_skip_n code1 8;
do token1 <- try_first_n code2 8;
let uvar3_1 := read_rm token1 in
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Psetcc (uvar4_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do code2 <- try_skip_n code1 8;
do token1 <- try_first_n code2 8;
let uvar3_1 := read_reg_op token1 in
if assertLength uvar3_1 3 then
let uvar3_2 := read_rm token1 in
if assertLength uvar3_2 3 then
do code3 <- try_skip_n code2 8;
OK ((Pcmov (uvar4_0) (uvar3_1) (uvar3_2)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Ptestl_EvGv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Ptestl_ri (uvar3_0) (uvar32_1)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Pcmpl_ri (uvar3_0) (uvar32_1)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pcmpl_GvEv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pcmpl_EvGv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar8_1 8 then
do code3 <- try_skip_n code2 8;
OK ((Prorl_ri (uvar3_0) (uvar8_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar8_1 8 then
do code3 <- try_skip_n code2 8;
OK ((Prolw_ri (uvar3_0) (uvar8_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
do token1 <- try_first_n code3 8;
let uvar8_2 := token1 in
if assertLength uvar8_2 8 then
do code4 <- try_skip_n code3 8;
OK ((Pshld_ri (uvar3_0) (uvar3_1) (uvar8_2)), 32)
else Error(msg"impossible")
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar8_1 8 then
do code3 <- try_skip_n code2 8;
OK ((Psarl_ri (uvar3_0) (uvar8_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar8_1 8 then
do code3 <- try_skip_n code2 8;
OK ((Pshrl_ri (uvar3_0) (uvar8_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar8_1 8 then
do code3 <- try_skip_n code2 8;
OK ((Psall_ri (uvar3_0) (uvar8_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pxorl_GvEv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pxorl_EvGv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Pxorl_ri (uvar3_0) (uvar32_1)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Porl_GvEv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Porl_EvGv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Porl_ri (uvar3_0) (uvar32_1)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Pandl_ri (uvar3_0) (uvar32_1)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pandl_GvEv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pandl_EvGv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar32_2 32 then
do code3 <- try_skip_n code2 32;
OK ((Pimull_ri (uvar3_0) (uvar3_1) (uvar32_2)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pimull_GvEv AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Psubl_GvEv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Psubl_EvGv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar32_1 32 then
do code3 <- try_skip_n code2 32;
OK ((Paddl_ri (uvar3_0) (uvar32_1)), 48)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pleal AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Pcvtsi2ss_d_fr (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Pcvttss_d_2si_rf (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code3 <- try_skip_n code2 8;
OK ((Pcvtsd2ss_d_ff (uvar3_0) (uvar3_1)), 24)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pmovsxd_GvEv AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pmovsw_GvEv AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pmovzw_GvEv AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pmovsb_GvEv AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pmovzb_rm AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pmovb_rm AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pmovb_mr AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
if assertLength uvar3_1 3 then
do code2 <- try_skip_n code1 8;
OK ((Pxchg_rr (uvar3_0) (uvar3_1)), 16)
else Error(msg"impossible")
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pflds_m AddrE), 8 + localLength0)
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pfstps_m AddrE), 8 + localLength0)
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pfstpl_m AddrE), 8 + localLength0)
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pfldl_m AddrE), 8 + localLength0)
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pmovss_d_fm AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code2;
do code3 <- try_skip_n code2 localLength0;
OK ((Pmovss_d_mf AddrE (uvar3_0)), 16 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pmovl_rm AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
do (AddrE, localLength0) <- decode_AddrE code1;
do code2 <- try_skip_n code1 localLength0;
OK ((Pmovl_mr AddrE (uvar3_0)), 8 + localLength0)
else Error(msg"impossible")
else
...
let code0 := bin in
do token0 <- try_first_n code0 8;
let uvar3_0 := read_col token0 in
if assertLength uvar3_0 3 then
do code1 <- try_skip_n code0 8;
do token1 <- try_first_n code1 32;
let uvar32_1 := token1 in
if assertLength uvar32_1 32 then
do code2 <- try_skip_n code1 32;
OK ((Pmovl_ri (uvar3_0) (uvar32_1)), 40)
else Error(msg"impossible")
else Error(msg"impossible")
else
