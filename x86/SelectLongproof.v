...
Definition partial_unary_constructor_sound (cstr: expr -> expr) (sem: val -> option val) : Prop :=
  ...
  eval_expr se ge sp e m le a x ->
  sem x = Some y ->
  exists v, eval_expr se ge sp e m le (cstr a) v /\ Val.lessdef y v.
...
Definition partial_binary_constructor_sound (cstr: expr -> expr -> expr) (sem: val -> val -> option val) : Prop :=
  ...
  eval_expr se ge sp e m le b y ->
  sem x y = Some z ->
  exists v, eval_expr se ge sp e m le (cstr a b) v /\ Val.lessdef z v.
...
Lemma is_longconst_sound:
  forall v a n le,
  is_longconst a = Some n -> eval_expr se ge sp e m le a v -> v = Vlong n.
...
Proof.
  ...
  red; intros.
  assert (DEFAULT: exists v, eval_expr se ge sp e m le (Eop Oorl (a:::b:::Enil)) v /\ Val.lessdef (Val.orl x y) v) by TrivialExists.
...
Proof.
  ...
  assert (DEFAULT: exists v, eval_expr se ge sp e m le (Eop (Oshllimm n) (a:::Enil)) v
...
Proof.
  ...
  destruct (Int.ltu n Int64.iwordsize') eqn:LT; simpl.
  assert (DEFAULT: exists v, eval_expr se ge sp e m le (Eop (Oshrluimm n) (a:::Enil)) v
...
Proof.
  ...
  destruct (Int.ltu n Int64.iwordsize') eqn:LT; simpl.
  assert (DEFAULT: exists v, eval_expr se ge sp e m le (Eop (Oshrlimm n) (a:::Enil)) v
...
- set (le' := x :: le).
  assert (A0: eval_expr se ge sp e m le' (Eletvar O) x) by (constructor; reflexivity).
