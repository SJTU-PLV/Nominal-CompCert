...
Program Definition semantics (p: program): Smallstep_.semantics li_asm li_asm :=
  {|
    ...
    activate se :=
      ...
      {|
        ...
        Smallstep_.after_external '(nb, s) r '(nb', s') := after_external nb s r s' /\ nb' = nb;
        Smallstep_.final_state '(nb, s) := final_state s;
...
Qed.
Next Obligation.
(* Same problem with calling convention of C *)
Admitted.
Next Obligation.
