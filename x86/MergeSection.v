...
Section WITH_SEC_REL_TBL.
  Context (sectbl: sectable) (reltbl:reloctable_map) (sec_id: ident).
...
Definition merge_text_sec (idlist: list ident) (symbtbl: symbtable) : res (section* symbtable * reloctable) :=
...
Definition transf_program (p: program) : res program :=
  ...
  do r1 <- merge_text_sec sectbl reltbl text_id idlist_text symbtbl;
  let '(text_sec, symbtbl1, text_reloctbl) := r1 in
  do r2 <- merge_data_sec sectbl reltbl data_id idlist_data symbtbl1;
  let '(data_sec, symbtbl2, data_reloctbl) := r2 in
  do r3 <- merge_rodata_sec sectbl reltbl rodata_id idlist_rodata symbtbl2;
  let '(rodata_sec, symbtbl3, rodata_reloctbl) := r3 in
  let sectbl1 := PTree.set text_id text_sec (PTree.empty section) in
  let sectbl2 := PTree.set data_id text_sec sectbl1 in
  let sectbl3 := PTree.set rodata_id text_sec sectbl2 in
