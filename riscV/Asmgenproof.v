...
Lemma transf_function_no_overflow:
  forall f tf,
  transf_function instr_size f = OK tf -> code_size instr_size tf.(fn_code) <= Ptrofs.max_unsigned.
Proof.
...
Proof.
  ...
  destruct Archi.ptr64. apply opimm64_label; TailNoLabel;constructor. apply opimm32_label; TailNoLabel; constructor.
Qed.
...
Lemma find_label_goto_label:
  ...
  exists tc', exists rs',
    goto_label instr_size tf lbl rs m = Next rs' m
...
Lemma exec_straight_steps_goto:
  ...
  (forall k c (TR: transl_instr f i ep k = OK c),
   exists jmp, exists k', exists rs2,
    ...
    /\ agree ms2 sp rs2
    /\ exec_instr instr_size tge tf jmp rs2 m2' = goto_label instr_size tf lbl rs2 m2') ->
  exists st',
  plus (step instr_size) tge (State rs1 m1') E0 st' /\
  match_states (Mach.State s fb sp c' ms2 m2) st'.
Proof.
...
Lemma exec_straight_opt_steps_goto:
  ...
  (forall k c (TR: transl_instr f i ep k = OK c),
   exists jmp, exists k', exists rs2,
       exec_straight_opt instr_size tge tf c rs1 m1' (jmp :: k') rs2 m2'
    /\ agree ms2 sp rs2
    /\ exec_instr instr_size tge tf jmp rs2 m2' = goto_label instr_size tf lbl rs2 m2') ->
  exists st',
  plus (step instr_size) tge (State rs1 m1') E0 st' /\
...
+ (* Indirect call *)
  ...
  f_equal. generalize (Ptrofs.eq_spec i Ptrofs.zero). rewrite Heqb0. auto.
  ...
  assert (TCA: transl_code_at_pc instr_size ge (Vptr fb (Ptrofs.add ofs (Ptrofs.repr (instr_size (Pjal_r x0 sig))))) fb f c false tf x).
    econstructor; eauto.
  exploit return_address_offset_correct; eauto. intros; subst ra.
  left; econstructor; split.
  apply plus_one. eapply exec_step_internal. rewrite H. auto. auto.  (* Simpl. rewrite <- H2; simpl; eauto. *)
...
+ (* Direct call *)
  generalize (code_tail_next_int instr_size instr_size_bound _ _ _ _ NOOV H7). intro CT1.
  assert (TCA: transl_code_at_pc instr_size ge (Vptr fb (Ptrofs.add ofs (Ptrofs.repr (instr_size (Pjal_s fid sig))))) fb f c false tf x).
    econstructor; eauto.
  exploit return_address_offset_correct; eauto. intros; subst ra.
  left; econstructor; split.
...
- (* Mtailcall *)
  ...
  destruct ros as [rf|fid]; simpl in H0; monadInv H10.
+ (* Indirect call *)
  assert (rs rf = Vptr (Global id) Ptrofs.zero).
  simpl in H0. repeat destr_in H0.
  f_equal. generalize (Ptrofs.eq_spec i Ptrofs.zero). rewrite Heqb0. auto.
    ...
    (* revert H; predSpec Ptrofs.eq Ptrofs.eq_spec i Ptrofs.zero; intros; congruence. *)
  assert (rs0 x0 = Vptr (Global id) Ptrofs.zero).
  exploit ireg_val; eauto. rewrite H10; intros LD; inv LD; auto.
  generalize H9. intros TF.
  monadInv H9. monadInv EQ0. destr_in EQ4. clear EQ2 EQ3 EQ4.
  exploit make_epilogue_correct; eauto. intros (rs1 & m1 & U & V & W & X & Y & Z).
  exploit exec_straight_steps_2;eauto. eauto using functions_transl.
  intros (ofs' & P & Q).
  left; econstructor; split.
  (* execution *)
  eapply plus_right'. eapply exec_straight_exec; eauto.
  econstructor. eexact P. eapply functions_transl; eauto. eapply find_instr_tail. auto. eexact Q.
  simpl. reflexivity.
  traceEq.
  (* match states *)
  econstructor; eauto.
...
+ (* Direct call *)
  generalize H9. intros TF.
  monadInv H9. monadInv EQ0. destr_in EQ3. clear EQ2 EQ3 EQ1.
  exploit make_epilogue_correct; eauto. intros (rs1 & m1 & U & V & W & X & Y & Z).
  ...
  (* execution *)
  eapply plus_right'. eapply exec_straight_exec; eauto.
  econstructor. eexact P. eapply functions_transl; eauto. eapply find_instr_tail.
...
- (* Mreturn *)
  ...
  assert (f0 = f) by congruence. subst f0.
  inversion AT; subst. simpl in H8; monadInv H8.
  assert (NOOV: code_size instr_size tf.(fn_code) <= Ptrofs.max_unsigned).  
  eapply transf_function_no_overflow; eauto.
  generalize H7. intros TF.
  monadInv H7. monadInv EQ0. destr_in EQ3. clear EQ2 EQ3 EQ1.
  exploit make_epilogue_correct; eauto. intros (rs1 & m1 & U & V & W & X & Y & Z).
  exploit exec_straight_steps_2; eauto using functions_transl.
  intros (ofs' & P & Q).
  left; econstructor; split.
...
- (* internal function *)
  ...
  destruct (zlt Ptrofs.max_unsigned (code_size instr_size x0.(fn_code))); inversion EQ1. clear EQ1. subst x0.
  unfold store_stack in *.
  exploit Mem.alloc_frame_extends. eauto. eauto.
  intros [m4' [K J]].  
  exploit Mem.alloc_extends. eauto. eauto. apply Z.le_refl. apply Z.le_refl.
  intros [m1' [C D]].
  exploit Mem.push_stage_extends;eauto. intros L.
  exploit Mem.record_frame_extends. eapply L. eauto.
  intros [m5' [M N]].  
  exploit Mem.storev_extends. eapply N.  eexact H3. eauto. eauto.
  intros [m2' [F G]].
  ...
  set (tfbody := Pallocframe (Mach.fn_stacksize f) (fn_retaddr_ofs f) (fn_link_ofs f) :: x0) in *.
  set (tf := {| fn_sig := Mach.fn_sig f; fn_code := tfbody ;fn_stacksize:= (Mach.fn_stacksize f)|}) in *.
  set (rs2 := nextinstr (Ptrofs.repr (instr_size (Pallocframe (Mach.fn_stacksize f) (fn_retaddr_ofs f) (fn_link_ofs f)))) (rs0#X30 <- (parent_sp s) #SP <- sp #X31 <- Vundef)).
  ...
  { change (fn_code tf) with tfbody; unfold tfbody.
    ...
    unfold exec_instr. destr.
    rewrite ATPC. rewrite K.
    rewrite C. fold sp. rewrite M.
    rewrite <- (sp_val _ _ _ AG). rewrite chunk_of_Tptr in F. rewrite ATLR. rewrite F.
    rewrite chunk_of_Tptr in P. rewrite P.
    reflexivity.
    reflexivity.
  }
  exploit exec_straight_steps_2; eauto using functions_transl. lia. constructor.
  intros (ofs' & X & Y).
  left; exists (State rs2 m3'); split.
...
Proof.
  ...
  constructor.
  simpl. auto with asmgen.
  simpl. congruence.
  intros. rewrite Regmap.gi. auto.
  eapply Genv.genv_vars_eq. eauto.
