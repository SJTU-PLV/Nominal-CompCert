    ...
    if zle 0 sz then
      ...
      |Vptr (Global id) _
       ...
       |Some m2 =>
          ...
          | None => Stuck
          | Some m3 =>
              match Mem.storev Mptr m3 (Val.offset_ptr sp ofs_link) rs#SP with
              | None => Stuck
              | Some m4 => Next (nextinstr (rs #X30 <- (rs SP) #SP <- sp #X31 <- Vundef)) m4
              end
          end
       end
     |_ => Stuck
      ...
      if zle 0 sz then
        ...
        | Some ra =>
            ...
            | Some sp =>
                match rs#SP with
                | Vptr stk ofs =>
                    (* the following checking may be used in the SSAsm proof *)
                    ...
                    | Some m' =>
                        ...
                        | Some m'' =>
                            ...
                            | None => Stuck
                            | Some m''' =>
                                Next (nextinstr (rs#SP <- sp #RA <- ra #X31 <- Vundef)) m'''
                            end
                        end                          
                    end
                | _ => Stuck
...
Definition instr_to_string i :=
  ...
  | Pfles     _ _ _ => "Pfles    "
  | Pfsqrts   _ _   => "Pfsqrts  "
  | Pfmadds   _ _ _ _ => "Pfmadds  "
  | Pfmsubs   _ _ _ _ => "Pfmsubs  "
  | Pfnmadds  _ _ _ _ => "Pfnmadds "
  | Pfnmsubs  _ _ _ _ => "Pfnmsubs "
  | Pfcvtws   _ _   => "Pfcvtws  "
  | Pfcvtwus  _ _   => "Pfcvtwus "
  | Pfcvtsw   _ _   => "Pfcvtsw  "
  | Pfcvtswu  _ _   => "Pfcvtswu "
  | Pfcvtls   _ _   => "Pfcvtls  "
  | Pfcvtlus  _ _   => "Pfcvtlus "
  | Pfcvtsl   _ _   => "Pfcvtsl  "
  | Pfcvtslu  _ _   => "Pfcvtslu "
  | Pfld      _ _ _ => "Pfld     "
  | Pfld_a    _ _ _ => "Pfld_a   "
  | Pfsd      _ _ _ => "Pfsd     "
  | Pfsd_a    _ _ _ => "Pfsd_a   "
  ...
  | Pfled     _ _ _ => "Pfled    "
  | Pfsqrtd   _ _   => "Pfsqrtd  "
  | Pfmaddd   _ _ _ _ => "Pfmaddd  "
  | Pfmsubd   _ _ _ _ => "Pfmsubd  "
  | Pfnmaddd  _ _ _ _ => "Pfnmaddd "
  | Pfnmsubd  _ _ _ _ => "Pfnmsubd "
  | Pfcvtwd   _ _  => "Pfcvtwd  "
  | Pfcvtwud  _ _  => "Pfcvtwud "
  | Pfcvtdw   _ _  => "Pfcvtdw  "
  | Pfcvtdwu  _ _  => "Pfcvtdwu "
  | Pfcvtld   _ _  => "Pfcvtld  "
  | Pfcvtlud  _ _  => "Pfcvtlud "
  | Pfcvtdl   _ _  => "Pfcvtdl  "
  ...
  | Ploadsi     _ _ => "Ploadsi  "
  | Pbtbl       _ _ => "Pbtbl   " 
  | Pbuiltin  _ _ _ => "Pbuiltin "
  | Pnop            => "Pnop     "
  | Plui_s    _ _ _ => "Plui_s   "
  | Paddi_s   _ _ _ _ => "Paddi_s " 
  | Pjal_ofs  _ _   => "Pjal_ofs" 
  | Pjal_rr   _ _ _ => "Pjal_rr  "
  | Pbeq_ofs  _ _ _ => "Pbeq_ofs "
  | Pbne_ofs  _ _ _ => "Pbne_ofs" 
  | Pblt_ofs  _ _ _ => "Pblt_ofs "
  | Pbltu_ofs  _ _ _ => "Pbltu_ofs" 
