...
Definition eval_branch (sz:ptrofs) (f: function) (l: label) (rs: regset) (m: mem) (res: option bool) : outcome :=
  match res with
    | Some true  => goto_label instr_size f l rs m
...
Definition exec_instr (ge: Genv.t) (f: function) (i: instruction) (rs: regset) (m: mem) : outcome :=
...
(** Unconditional jumps.  Links are always to X1/RA. *)
  | Pj_l l =>
      goto_label instr_size f l rs m
  | Pj_s s sg =>
...
(** Pseudo-instructions *)
  ...
  | Pbtbl r tbl =>
      ...
      | Vint n =>
          ...
          | None => Stuck
          | Some lbl => goto_label instr_size f lbl (rs#X5 <- Vundef #X31 <- Vundef) m
          end
      | _ => Stuck
      end
