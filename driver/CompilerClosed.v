...
Proof.
  intros. inv H0.
  destruct wB as ([a [b1 b2]] & [c [d1 d2]] & [e f] & g).
  destruct H as [r1 [Hr1 [r2' [Hr2 [r3 [Hr3 Hr4]]]]]].
  inv Hr1. inv Hr2. inv Hr3. inv Hr4. inv H. simpl in H0.
  destruct H1.
  red in H. inv H.
  destruct r2. inv H1.
  constructor. unfold Conventions1.loc_result in tres. subst tres.
  replace (Archi.ptr64) with true in H3 by reflexivity.
  unfold Conventions1.loc_result_64 in H3.
  assert (sg = main_sg). admit.
  rewrite H1 in H3. simpl in H3. simpl in H.
  inv H3. specialize (H RAX). rewrite <- H20 in H. inv H. reflexivity.
Admitted.
