...
Inductive injp_world :=
  injpw (f: meminj) (gs1 gs2: sup) (m1 m2: mem) (Hm: Mem.inject f m1 m2).
...
Inductive injp_acc: relation injp_world :=
  injp_acc_intro f m1 m2 Hm f' m1' m2' Hm' gs1 gs2:
    Mem.ro_unchanged m1 m1' -> Mem.ro_unchanged m2 m2' ->
    injp_max_perm_decrease m1 m1' ->
    injp_max_perm_decrease m2 m2' ->
    Mem.unchanged_on (fun b z => loc_unmapped f b z /\ ~ sup_In b gs1) m1 m1' ->
    Mem.unchanged_on (fun b z => loc_out_of_reach f m1 b z /\ ~ sup_In b gs2) m2 m2' ->
    inject_incr f f' ->
    inject_separated f f' m1 m2 ->
    injp_acc (injpw f gs1 gs2 m1 m2 Hm) (injpw f' gs1 gs2 m1' m2' Hm').

Definition injp_mi :=
  fun '(injpw f _ _ _ _ _) => f.
...
Inductive injp_match_stbls: injp_world -> relation Genv.symtbl :=
  injp_match_stbls_intro f m1 m2 Hm se1 se2:
    let gs1 := (Genv.genv_sup se1) in let gs2 := (Genv.genv_sup se2) in
    Genv.match_stbls f se1 se2 ->
    Mem.sup_include gs1 (Mem.support m1) ->
    Mem.sup_include gs2 (Mem.support m2) ->
    injp_match_stbls (injpw f gs1 gs2 m1 m2 Hm) se1 se2.
...
Proof.
  ...
  - intros w1 w2 w3 H12 H23.
    destruct H12 as [f m1 m2 Hm f' m1' m2' Hm' gs1 gs2 Hr1 Hr2 Hp1 Hp2 H1 H2 Hf Hs].
    ...
    + eapply mem_unchanged_on_trans_implies_valid; eauto.
      unfold loc_unmapped.
      intros b1 _ Hb Hb1. destruct Hb. split; eauto.
      destruct (f' b1) as [[b2 delta] | ] eqn:Hb'; eauto.
      edestruct Hs; eauto. contradiction.
    + eapply mem_unchanged_on_trans_implies_valid; eauto.
      unfold loc_out_of_reach.
      intros b2 ofs2 [Hptr2 Hg] Hb2.
      split; eauto.
      intros b1 delta Hb' Hperm.
...
Lemma injp_acc_alloc: forall f f' m1 m2 b1 b2 lo hi m1' m2' Hm Hm' gs1 gs2,
...
Lemma injp_acc_free: forall f m1 m2 b1 b2 delta lo1 sz m1' m2' Hm Hm' gs1 gs2,
...
Lemma injp_acc_store : forall f chunk v1 v2 b1 b2 ofs1 delta m1 m2 m1' m2' Hm Hm' gs1 gs2,
...
Lemma injp_acc_storev : forall f chunk v1 v2 a1 a2 m1 m2 m1' m2' Hm Hm' gs1 gs2,
...
Next Obligation. (* ~> vs. match_stbls *)
  ...
  destruct Hse as [f m1 m2 Hm se1 se2 gs1 gs2 Hse Hnb1 Hnb2]. inv Hw'.
  constructor.
  - eapply Genv.match_stbls_incr; eauto.
    intros b1 b2 delta Hb Hb'. specialize (H12 b1 b2 delta Hb Hb').
    unfold Mem.valid_block in H12. split; inv H12; eauto.
  - apply Mem.unchanged_on_support in H9. eauto.
...
Next Obligation. (* Mem.alloc *)
  intros _ _ _ [f gs1 gs2 m1 m2 Hm] lo hi.
...
Next Obligation. (* Mem.free *)
  intros _ _ _ [f gs1 gs2 m1 m2 Hm] [[b1 lo1] hi1] [[b2 lo2] hi2] Hr.
  simpl. red.
...
Next Obligation. (* Mem.load *)
  intros _ chunk _ _ [f gs1 gs2 m1 m2 Hm] _ _ [b1 ofs1 b2 delta Hptr].
  simpl. red.
...
Next Obligation. (* Mem.store *)
  intros _ chunk _ _ [f gs1 gs2 m1 m2 Hm] _ _ [b1 ofs1 b2 delta Hptr] v1 v2 Hv.
  simpl in *. red.
...
Next Obligation. (* Mem.loadbytes *)
  intros _ _ _ [f gs1 gs2 m1 m2 Hm] _ _ [b1 ofs1 b2 delta Hptr] sz.
  simpl. red.
...
Next Obligation. (* Mem.storebytes *)
  intros _ _ _ [f gs1 gs2 m1 m2 Hm] [b1 ofs1] [b2 ofs2] Hptr vs1 vs2 Hvs.
  simpl. red.
...
Proof.
  intros w se1 se2 m1 m2 Hse Hm. destruct Hm as [f gs1 gs2 m1 m2 Hm].
  exists (injpw (meminj_dom f) gs1 gs1 m1 m1 (mem_inject_dom f m1 m2 Hm),
          injpw f gs1 gs2 m1 m2 Hm); simpl.
  repeat apply conj.
  ...
  - intros [w12' w23'] m1' m3' (m2' & H12' & H23') [Hw12' Hw23']. cbn in *.
    ...
    rename gs4 into gs1. rename gs5 into gs2.
    eexists (injpw (compose_meminj f12' f23') gs1 gs2 m1' m3'
    ...
    + constructor; auto.
      *
        ...
        intros. apply unchanged_on_perm. destruct H11.
        split. red. red in H11. unfold meminj_dom. rewrite H11. reflexivity.
        eauto. eauto.
        intros. apply unchanged_on_contents.  destruct H11.
        split. red. red in H11. unfold meminj_dom. rewrite H11. reflexivity.
        eauto. eauto.
...
Section CONSTR_PROOF.
  ...
  Let gs1 := Genv.genv_sup se1.
  Let gs2 := Genv.genv_sup se2.
  Let gs3 := Genv.genv_sup se3.
   ...
   *)
  Hypothesis MSTBL12 : Genv.match_stbls' j1 se1 se2.
  Hypothesis MSTBL23 : Genv.match_stbls' j2 se2 se3.
  Hypothesis GSUP1 : Mem.sup_include gs1 (Mem.support m1).
  Hypothesis GSUP2 : Mem.sup_include gs2 (Mem.support m2).
  Hypothesis GSUP3 : Mem.sup_include gs3 (Mem.support m3).
...
Qed.
  ...
  Lemma unchanged_on_copy_block2 : forall m m' b gs,
      Mem.copy_block m1 m2 m1' j1 j2 j1' INJ12 b gs m = m' ->
      Mem.unchanged_on (fun b ofs => loc_unmapped j2 b ofs /\ ~ sup_In b gs ) m m'.
  Proof.
    ...
    constructor; simpl. eauto with mem.
    intros. destruct H. unfold Mem.perm. simpl. erewrite pmap_update_diff'. reflexivity.
    congruence.
    intros. destruct H. rewrite pmap_update_diff'. reflexivity.
    congruence.
    - (*global*)
      destruct (Mem.sup_dec b gs) eqn:SUP; eauto with mem.
      destruct (Mem.sup_dec b (Mem.support m)); eauto with mem.
      constructor; simpl. eauto with mem.
      intros. destruct H. unfold Mem.perm. simpl. erewrite pmap_update_diff'. reflexivity.
      congruence.
      intros. destruct H. rewrite pmap_update_diff'. reflexivity.
  ...
  Qed.
    ...
    Lemma unchanged_on_copy_block1 : forall m m' b gs,
      Mem.copy_block m1 m2 m1' j1 j2 j1' INJ12 b gs m = m' ->
      Mem.unchanged_on (fun b ofs => loc_out_of_reach j1 m1 b ofs /\ ~ sup_In b gs) m m'.
  Proof.
    intros. subst. unfold Mem.copy_block.
    destruct (j2 b) as [[b3 d]|] eqn: j2b; eauto with mem.
    -
    ...
    + intros. unfold Mem.perm. simpl.
      ...
      destruct LOCIN as [A B]. destruct H.
      red in H. exploit H; eauto. replace (ofs - (ofs - o1)) with o1 by lia.
      eauto. intro. inv H2. reflexivity. reflexivity.
    + intros. unfold Mem.perm. simpl.
      unfold Mem.pmap_update.
      rewrite NMap.gsspec.
      destruct (eq_block). subst.
  ...
  Lemma unchanged_on_copy'1 : forall s m m' gs,
      Mem.copy_sup m1 m2 m1' j1 j2 j1' INJ12 s gs m = m' ->
      Mem.unchanged_on (fun b ofs => loc_out_of_reach j1 m1 b ofs /\ ~ sup_In b gs) m m'.
  Proof.
  ...
  Lemma unchanged_on_copy'2 : forall s m m' gs,
      Mem.copy_sup m1 m2 m1' j1 j2 j1' INJ12 s gs m = m' ->
      Mem.unchanged_on (fun b ofs => loc_unmapped j2 b ofs /\ ~ sup_In b gs) m m'.
  Proof.
    induction s; intros; subst; simpl.
  ...
  Lemma unchanged1_step3: Mem.unchanged_on (fun b ofs => loc_out_of_reach j1 m1 b ofs /\ ~ sup_In b gs2) m2'1 m2'2.
  Proof.
    unfold m2'2.
    eapply unchanged_on_copy'1; eauto.
  Qed.

  Lemma unchanged2_step3: Mem.unchanged_on (fun b ofs => loc_unmapped j2 b ofs /\ ~ sup_In b gs2) m2'1 m2'2.
  Proof.
    unfold m2'2.
  ...
  Theorem UNCHANGE21': Mem.unchanged_on (fun b ofs => loc_out_of_reach j1 m1 b ofs /\ ~ sup_In b gs2) m2 m2'2.
  Proof.
    eapply Mem.unchanged_on_trans.
    eapply Mem.unchanged_on_implies.
    eapply unchanged1_step2. intros. apply H.
  ...
  (** Lemma C.8(2) *)
  Theorem UNCHANGE22' : Mem.unchanged_on (fun b ofs => loc_unmapped j2 b ofs /\ ~ sup_In b gs2 ) m2 m2'2.
  Proof.
    eapply Mem.unchanged_on_trans; eauto.
    eapply Mem.unchanged_on_implies.
    eapply unchanged2_step2. intros. apply H.
  ...
  Lemma copy_block_perm1 : forall m b1 o1 b2 o2 k p,
      ...
      (j2 b2 <> None \/ sup_In b2 gs2) ->
      Mem.support m = s2' ->
      Mem.perm (Mem.copy_block m1 m2 m1' j1 j2 j1' INJ12 b2 gs2 m) b2 o2 k p <-> Mem.perm m1' b1 o1 k p.
  Proof.
    intros.
    ...
    - destruct (Mem.sup_dec b2 gs2) eqn: SUP; try congruence.
      destruct (Mem.sup_dec b2 (Mem.support m)).
    + unfold Mem.perm. simpl. unfold Mem.pmap_update.
      setoid_rewrite NMap.gss. rewrite Mem.copy_access_block_result.
      destruct Mem.loc_in_reach_find as [[b1' o1']|]eqn:FIND.
      apply Mem.loc_in_reach_find_valid in FIND. destruct FIND as [A B].
  ...
  Lemma copy_block_perm2 : forall m b2 o2 b2' k p,
      b2 <> b2' ->
      Mem.perm (Mem.copy_block m1 m2 m1' j1 j2 j1' INJ12 b2' gs2 m) b2 o2 k p <-> Mem.perm m b2 o2 k p.
  Proof.
    intros.
  ...
  Lemma copy_sup_perm: forall s m b1 o1 b2 o2 k p,
        ...
        sup_In b2 s ->
        Mem.support m = s2' ->
        Mem.perm (Mem.copy_sup m1 m2 m1' j1 j2 j1' INJ12 s gs2 m) b2 o2 k p <-> Mem.perm m1' b1 o1 k p.
  Proof.
    induction s; intros.
    - inv H2.
    - simpl. destruct H2.
...
*)
      (j2 b2 <> None \/ sup_In b2 gs2) ->
      Mem.support m = s2' ->
      mem_memval (Mem.copy_block m1 m2 m1' j1 j2 j1' INJ12 b2 gs2 m) b2 o2 =
  ...
  Lemma copy_block_content1 : forall m b1 o1 b2 o2,
      ...
      (j2 b2 <> None \/ sup_In b2 gs2 ) ->
      Mem.support m = s2' ->
      mem_memval (Mem.copy_block m1 m2 m1' j1 j2 j1' INJ12 b2 gs2 m) b2 o2 = Mem.memval_map j1' (mem_memval m1' b1 o1).
  Proof.
  ...
  Lemma copy_block_content3 : forall m b2 o2 b2',
      b2 <> b2' ->
      mem_memval (Mem.copy_block m1 m2 m1' j1 j2 j1' INJ12 b2' gs2 m) b2 o2 = mem_memval m b2 o2.
  Proof.
    intros.
    unfold Mem.copy_block. destruct (j2 b2'); try reflexivity.
    destruct Mem.sup_dec; try reflexivity.
  ...
  Lemma copy_sup_content: forall s m b1 o1 b2 o2,
        ...
        sup_In b2 s ->
        Mem.support m = s2' ->
        mem_memval (Mem.copy_sup m1 m2 m1' j1 j2 j1' INJ12 s gs2 m) b2 o2 = Mem.memval_map j1' (mem_memval m1' b1 o1).
  Proof.
    induction s; intros.
    - inv H3.
    - simpl. destruct H3.
  ...
  Lemma copy_sup_content_2: forall s m b1 o1 b2 o2,
        ...
        (j2 b2 <> None \/ sup_In b2 gs2) ->
        Mem.support m = s2' ->
        mem_memval (Mem.copy_sup m1 m2 m1' j1 j2 j1' INJ12 s gs2 m) b2 o2 = mem_memval m b2 o2.
  Proof.
  ...
  Proof.
    intros.
    intro. inv MSTBL23. exploit mge_dom; eauto.
  ...
  Proof.
    ...
    - eapply Mem.loc_in_reach_find_valid in LOCIN; eauto.
      ...
      + generalize (UNCHANGE22'). intro UNC2.
        inversion UNC2. eapply unchanged_on_perm; eauto.
        apply perm_decrease_step4. eauto.
   ...
   (* eapply Mem.ro_unchanged_trans.
    ...
    - eapply Mem.loc_in_reach_find_valid in LOCIN; eauto. destruct LOCIN as [MAP1 PERM1].
      ...
      + generalize UNCHANGE22'. intro UNC22. split; inv UNC22.
        ...
        eapply perm_decrease_step4; eauto.
        symmetry. erewrite <- unchanged_on_contents; eauto.
        apply unchanged_content_step4.
    ...
    - constructor.
      + intros.
        ...
        * destruct (j2 b2) as [[b3 delta2]|] eqn:j2b2.
          ...
          -- assert (Hv1: Mem.valid_block m1 b1). unfold Mem.valid_block.
             eapply DOMIN1. eauto. 
             assert (Hpmax1: Mem.perm m1 b1 ofs Max Nonempty).
             apply MAXPERM1; eauto with mem.
            generalize UNCHANGE22'. intro UNCHANGE22'.
             ...
             destruct A as [A B]. *)
             inversion UNCHANGE22'.
             generalize unchanged_step4. intro UNCS4.
...
Proof.
  ...
  - intros w13' m1' m3' MMEM13' INCR13'.
    unfold rel_compose.
    (* clear MSTBL13. *)
    destruct MSTBL13 as [se2 [MSTBL12 MSTBL23]].
    assert (gs0 = gs2). inv MSTBL12. inv MSTBL23. unfold gs8. unfold gs7.
    reflexivity. subst gs0.
    cbn in INCR13'.
    inv MMEM13'. rename f into j13'. rename Hm3 into INJ13'.
    cbn.
    inversion INCR13' as [? ? ? ? ? ? ? ? ? ?
                            ROUNC1 ROUNC3 MAXPERM1 MAXPERM3 UNCHANGE1 UNCHANGE3 INCR13 DISJ13]. subst.
