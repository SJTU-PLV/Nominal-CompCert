...
(* used in transf_initial_state *)
Lemma decode_program_equiv: forall D (p1 p2: RelocProg.program fundef unit instruction D),
    program_equiv p1 p2 ->
    program_equiv (decode_program instr_size p1) (decode_program instr_size p2).
Proof.
  intros.
  unfold decode_program. econstructor;simpl;eauto;intros.
  eapply pe_symbtable;eauto.
  repeat erewrite PTree.gmap.
  erewrite pe_sectable;eauto. f_equal.
  unfold rev_section. erewrite pe_reloctable_map;eauto.
  apply pe_reloctable_map;eauto.
...
Lemma program_equiv_symbol_address: forall D (p1 p2: RelocProg.program fundef unit instruction D),
    program_equiv p1 p2 ->
    forall id ofs, RelocProgGlobalenvs.Genv.symbol_address (RelocProgGlobalenvs.globalenv instr_size p1) id ofs = RelocProgGlobalenvs.Genv.symbol_address (RelocProgGlobalenvs.globalenv instr_size p2) id ofs.
Proof.
  intros.
  unfold RelocProgGlobalenvs.globalenv. unfold RelocProgGlobalenvs.Genv.symbol_address.
...
Lemma program_equiv_ext_funs: forall D (p1 p2: RelocProg.program fundef unit instruction D),
    program_equiv p1 p2 ->
    Genv.genv_ext_funs (RelocProgGlobalenvs.globalenv instr_size p1) = Genv.genv_ext_funs (RelocProgGlobalenvs.globalenv instr_size p2).
Proof.
  unfold RelocProgGlobalenvs.globalenv.
