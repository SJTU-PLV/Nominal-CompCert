...
(* just initialize the location (b, ph), return the new allocated
blocks *)
Fixpoint shallow_initialize (shr_map: TyMap.t (option ablock)) (rec_map: TyMap.t (option aval)) (ty: type) (b: ablock) (ph: path) (m: amem) (next_extern: positive) (next_tag: positive) : res (amem * positive * positive * TyMap.t (option ablock) * TyMap.t (option aval) * (list ablock)) :=
  ...
  | None =>
      ...
      | Tbox ty' _ | Treference ty' Mutable _ =>
          ...
          (* return (b', ty') *)
          OK (m3, Pos.succ next_extern, Pos.succ next_tag, shr_map, [b'])
      | Treference ty' Immutable _ =>
          ...
          | None =>
              ...
              do (b', m1) <- alloc_external_block ce m ty' next_extern;
              (* update the borrow tree in b' *)
              do (ap, m2) <- create_reference_from_owner Immutable b' [] (Textern next_tag) m1;
              (* the abstract pointer is (b', [], Textern next_tag) *)
              let av := Ptr (Aptrs.singleton ap) in
              do m3 <- store m2 b ph av;
              (* return (b', ty') and update the share map. But in
              this moment b' is not initialized and we add b' to
              shr_map. Is it correct? *)
              OK (m3, Pos.succ next_extern, Pos.succ next_tag, (TyMap.set ty' b'), [b'])
          end
      | Tstruct id _
      | Tvariant id _ =>
          match ce!id with
...
Fixpoint deep_initialize (shr_map: TyMap.t (option ablock)) (rec_map: TyMap.t (option aval)) (ty: type) (b: ablock) (m: amem) (next_extern: positive) (next_tag: positive) : res (amem * (positive * positive) * TyMap.t (option ablock)) :=
  (* check whether ty is a recursive type and is associated with an
  initial abstract value *)
  match TyMap.get ty rec_map with
  | Some v =>
  ...
  | None =>       
      ...
      | Tbox ty' _ | Treference ty' Mutable _ =>
          ...
          do m3 <- store m2 b [] av;
          (* initialize b' *)
          deep_initialize shr_map rec_map ty' b' m3 (Pos.succ next_extern) (Pos.succ next_tag)
      | Treference ty' Immutable _ =>
          (* if [&ty'] has appeared and has been allocated *)
          match TyMap.get ty' shr_map with
          | Some b' =>
          ...
          | None =>
              ...
              deep_initialize (TyMap.set ty' (Some b')) rec_map ty' b' m3 (Pos.succ next_extern) (Pos.succ next_tag)
          end
      | Tstruct id _ =>
