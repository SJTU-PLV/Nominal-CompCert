...
Lemma tr_move_correct:
  forall r1 ns r2 nd cs f sp rs m st,
  tr_move f.(fn_code) ns r1 nd r2 ->
  exists rs',
  star step tge (State cs f sp ns rs m st) E0 (State cs f sp nd rs' m st) /\
  rs'#r2 = rs#r1 /\
...
Definition transl_expr_prop
  ...
  forall tm cs f map pr ns nd rd rs dst st
    (MWF: map_wf map)
    (TE: tr_expr f.(fn_code) map pr a ns nd rd dst)
    (ME: match_env map e le rs)
    (EXT: Mem.extends m tm),
  exists rs', exists tm',
     star step tge (State cs f sp ns rs tm st) E0 (State cs f sp nd rs' tm' st)
...
Definition transl_exprlist_prop
  ...
  forall tm cs f map pr ns nd rl rs st
    (MWF: map_wf map)
...
Definition transl_condexpr_prop
  ...
  forall tm cs f map pr ns ntrue nfalse rs st
    (MWF: map_wf map)
    (TE: tr_condition f.(fn_code) map pr a ns ntrue nfalse)
    (ME: match_env map e le rs)
    (EXT: Mem.extends m tm),
  exists rs', exists tm',
     plus step tge (State cs f sp ns rs tm st) E0 (State cs f sp (if v then ntrue else nfalse) rs' tm' st)
  /\ match_env map e le rs'
  /\ (forall r, In r pr -> rs'#r = rs#r)
  /\ Mem.extends m tm'.
...
Definition transl_exitexpr_prop
  ...
  forall tm cs f map ns nexits rs st
    (MWF: map_wf map)
...
Definition measure_state (S: CminorSel.state) :=
  match S with
  | CminorSel.State _ s k _ _ _ _ => (size_stmt s + size_cont k, size_stmt s)
  | _                           => (0, 0)
  end.
...
Lemma lt_state_intro:
  forall f1 s1 k1 sp1 e1 m1 f2 s2 k2 sp2 e2 m2 st,
  size_stmt s1 + size_cont k1 < size_stmt s2 + size_cont k2
  \/ (size_stmt s1 + size_cont k1 = size_stmt s2 + size_cont k2
      /\ size_stmt s1 < size_stmt s2) ->
  lt_state (CminorSel.State f1 s1 k1 sp1 e1 m1 st)
           (CminorSel.State f2 s2 k2 sp2 e2 m2 st).
Proof.
...
Inductive match_states: CminorSel.state -> RTL.state -> Prop :=
  | match_state:
      forall f s k sp e m tm cs tf ns rs map ncont nexits ngoto nret rret st
        (MWF: map_wf map)
      ...
      match_states (CminorSel.State f s k sp e m st)
                   (RTL.State cs tf sp ns rs tm st)
  | match_callstate:
      forall f args targs k m tm cs tf id st
        (TF: transl_fundef f = OK tf)
        (MS: match_stacks k cs)
        (LD: Val.lessdef_list args targs)
        (MEXT: Mem.extends m tm),
      match_states (CminorSel.Callstate f args k m st id)
                   (RTL.Callstate cs tf targs tm st id)
  | match_returnstate:
      forall v tv k m tm cs st
        (MS: match_stacks k cs)
        (LD: Val.lessdef v tv)
        (MEXT: Mem.extends m tm),
      match_states (CminorSel.Returnstate v k m st)
                   (RTL.Returnstate cs tv tm st).
