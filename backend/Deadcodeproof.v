...
Inductive match_states: state -> state -> Prop :=
  ...
  | match_call_states:
      forall s vf args m ts tvf targs tm id
        (STACKS: list_forall2 match_stackframes s ts)
...
- (* call *)
  TransfInstr; UseTransfer.
  exploit ros_address_translated; eauto 2 with na.
  intro FLD.
  exploit functions_translated; eauto.
  intros (tfd & A & B).
  econstructor; split.
  eapply exec_Icall; eauto.
  cbn in FLD. subst vf. rewrite H0 in FLD. inv FLD. eauto.
...
- (* tailcall *)
  ...
  exploit ros_address_translated; eauto 2 with na.
  intro FLD.
  exploit functions_translated. eauto. subst vf. eauto.
  intros (tfd & A & B).
...
Proof.
  ...
  exists (Callstate nil (Vptr (Global id) Ptrofs.zero) vargs2 m2 id); split.
  setoid_rewrite <- (sig_function_translated _ _ _ B). monadInv B. inv H.
  econstructor; eauto.
