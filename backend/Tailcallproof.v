...
Proof.
  ...
  rewrite H5. eapply functions_translated. eauto. 2:eauto.
  rewrite <- H5 at 2. apply H2.
  destr; intros. destr.
  rewrite <- Genv.find_funct_find_funct_ptr in H.
  erewrite <- functions_translated, Genv.find_funct_find_funct_ptr. 1,2,4:eauto.
  edestruct @Genv.find_symbol_match as (b' & Hb' & H'); eauto.
  inv H0. exploit H3; eauto. intro. rewrite H0 in Hb'. inv Hb'.
  rewrite H' in Heqo0. inv Heqo0. econstructor; eauto.
  edestruct @Genv.find_symbol_match as (b' & Hb' & H'); eauto. congruence.
Qed. *)
...
Lemma eval_builtin_arg_inject:
  ...
  /\ Val.inject j v v'.
Proof.
  induction 1; intros SP GL RS MI GE0.
...
- econstructor; split; eauto with barg. apply symbols_preserved. auto.
...
Section TAKEDROP.
  ...
  Proof.
    ...
    exploit IHn; eauto. intro.
    destruct l. simpl in H. rewrite drop_nil in H. congruence.
  ...
  Lemma drop_take:
    forall n l,
      drop n l <> nil ->
      exists t, take n l = Some t.
  Proof. intros. auto using take_succeeds, drop_length_leq. Qed.

  Lemma drop_take':
    forall n l hd tl,
      hd :: tl = drop n l ->
      exists t, take n l = Some t.
  Proof. intros. apply drop_take. congruence. Qed.
...
| sizes_nil: forall
    ...
    tc_sizes nil (Mem.astack (injw_sup_l w)) (Mem.astack (injw_sup_r w))
| sizes_cons: forall l s1 s2 t1 n t2
    (IHsize: tc_sizes l (drop (S n) s1) s2)
    (TAKE: take (S n) s1 = Some t1)
    (SIZE: stack_size t1 = size_of_all_frames t2),
...
Proof.
  ...
  econstructor; simpl; auto.
  rewrite Heqo. eauto.
  cbn in *. unfold frame_size_a. cbn. lia.
Qed.

Lemma size_of_all_frames_app : forall s1 s2,
    size_of_all_frames (s1++s2) = size_of_all_frames s1 + size_of_all_frames s2.
Proof.
...
(** Here is the invariant relating two states.  The first three
  ...
  over values and register states, and the corresponding ``inject''
...
Inductive match_states: state -> state -> Prop :=
  ...
  | match_states_interm:
      forall s sp pc rs m s' m' f r v' j n l
             (STACKS: match_stackframes j n l s s')
             (* (TAKE: take (S n) (Mem.astack (Mem.support m)) = Some t1)
      ...
      f.(fn_stacksize) = 0 ->
      Val.inject j (rs#r) v' ->
      inj_incr_without_astack w (injw j (Mem.support m) (Mem.support m')) ->
      match_states (State s f (Vptr sp Ptrofs.zero) pc rs m)
                   (Returnstate s' v' m').
...
Lemma inj_incr_clearstage:
  forall j m m' m'' s,
  inj_incr_without_astack w (injw j (Mem.support m) s) ->
  Mem.support m = Mem.support m' ->
  Mem.pop_stage m' = Some m'' ->
  inj_incr_without_astack w (injw j (Mem.support (Mem.push_stage m'')) s).
Proof.
...
- (* op *)
  ...
  rewrite eval_shift_stack_operation in EVAL'. auto.
...
- (* load *)
  ...
  rewrite eval_shift_stack_addressing in ADDR'. auto. auto.
  econstructor; eauto. apply set_reg_inject; auto.
...
- (* store *)
  ...
  rewrite eval_shift_stack_addressing in ADDR'. rewrite <- ADDR'. auto. auto.
...
+ (* call turned tailcall *)
  ...
  left. exists (Callstate s' (ros_address tge ros rs') (rs'##args) m'' id); split.
  ...
  apply Mem.push_stage_left_inject. eapply Mem.free_right_inject; eauto.
...
+ (* call that remains a call *)
  ...
  apply tc_sizes_up. auto. destruct w. inv INCR. constructor; auto.
...
- (* tailcall *)
  ...
  TransfInstr.
  left. exists (Callstate s' (ros_address tge ros rs') (rs'##args) m'1 id); split.
...
- (* builtin *)
  ...
  apply Mem.unchanged_on_support in E. apply E in H6.
  rewrite Mem.support_pop_stage_1 in H6; eauto.
...
- (* internal call *)
  ...
  rewrite z. rewrite d.  apply tc_sizes_record.
  rewrite H in c. rewrite ALLOC in y. unfold sup_incr in c,y.
  simpl in c,y. congruence. congruence.
...
Proof.
  ...
  - econstructor.
    ...
    rewrite POP1 in H3. simpl in H3.
    eapply Mem.astack_pop_stage in e as H2'. destruct H2' as [hd2 POP2].
    rewrite POP2 in H4. simpl in H4.
    split; constructor.
    1,2,3,8:eauto.
...
Proof.
  ...
  - constructor; simpl; auto. constructor; auto.
    ...
    congruence.
  - inv GE. inv INCR.
    rewrite <- H1 in inj_stbls_next_l.
    rewrite <- H1 in inj_stbls_next_r.
    simpl in *. constructor; simpl. auto.
