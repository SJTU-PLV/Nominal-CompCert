...
Lemma globalenv_match_stbls m se tse j m1:
...
Proof.
  intros until path. intros AGREGS AGCS AGARGS WTREGS LS1 RS1 FIND ALLOCF ALLOC TYPAR TYRA SEP SUP HG.
...
Local Opaque b fe.
  ...
  intros (A & B). subst path'.
  exploit alloc_frame_parallel_rule_2. eexact SEP. eauto. eexact ALLOCF'.
  clear SEP. intro SEP.
  ...
  clear SEP SEPCONJ.
  assert (SUP'': Mem.stackseq m2 m5').
   apply Mem.support_storev in STORE_RETADDR.
   apply Mem.support_storev in STORE_PARENT.
   unfold Mem.stackseq.
   rewrite <- SUP'. rewrite <- STORE_RETADDR. rewrite <- STORE_PARENT.
   eapply Mem.alloc_parallel_stackseq; eauto.
   assert (SINJ' : same_at_glob j').
   admit. (* preseve of incr injection *)
...
Inductive match_states: Linear.state -> Mach.state -> Prop :=
  ...
  | match_states_call:
      forall cs vf f ls m cs' vf' rs m' j id
        (NB: Mem.sup_include (injw_sup_l (stk_w w)) (Mem.support m))
...
>>>>>>> a091c4c *)
  ...
  unfold store_stack in STORE.
  unfold Mem.stackseq. rewrite <- (Mem.support_storev _ _ _ _ _ STORE).
  eauto. eauto. eauto.
...
- (* Lstore *)
  ...
  auto. eauto with coqlib. unfold Mem.stackseq.
...
- (* Lcall *)
  ...
  subst vf. rewrite H in H1.
  inversion H1. subst.
  apply SINJ in H5. destr_in H5. eauto.
  assert (Genv.match_stbls j se tse).
  eapply globalenv_match_stbls. eapply sep_proj2.
  eapply sep_proj2. eapply sep_proj2.  eexact SEP.
  exploit symbol_address_inject; eauto. subst vf. rewrite H.
  intro. inv H2. apply SINJ in H6. destr_in H6. eauto.
  econstructor; eauto.
  econstructor; eauto with coqlib.
...
- (* Ltailcall *)
  ...
  apply Mem.support_free in R as SF'.
  apply Mem.support_free in H3 as SF.
  assert (SEQ: Mem.stackseq m' m1'). unfold Mem.stackseq in *.
  congruence.
  exploit Mem.return_frame_parallel_stackseq; eauto.
  intros (m2' & RET & SEQ'').
  exploit return_frame_parallel_rule_2; eauto.
  clear SEP. intro SEP.
  rewrite sep_swap in SEP.
  exploit find_function_translated; eauto.
    eapply sep_proj2. eapply sep_proj2. eexact SEP.
  ...
  instantiate (1:= id).
  destruct ros; simpl in *; eauto.
  generalize (T m0). simpl. intro.
  subst vf. rewrite H in H0.
  inv H0. apply SINJ in H7. destr_in H7. eauto.
  assert (Genv.match_stbls j se tse).
  eapply globalenv_match_stbls. eapply sep_proj2.
  eapply sep_proj2.  eexact SEP.
  exploit symbol_address_inject; eauto. subst vf. rewrite H.
  intro. inv H2. apply SINJ in H8. destr_in H8. eauto.
  traceEq.
  econstructor; eauto.
  eapply Mem.sup_include_trans with (Mem.support m').
  ...
  rewrite <- SF. eapply Mem.sup_include_return_frame; eauto.
...
- (* Lbuiltin *)
  ...
  clear SEP; intros (res' & m1' & EC & RES & UNCH & SEP & MINJ).
  rewrite <- sep_assoc, sep_comm, sep_assoc in SEP.
  econstructor; split.
  apply plus_one. econstructor; eauto.
  ...
  apply external_call_support in EC.
  apply external_call_support in H0.
  congruence.
(*{
  apply Axioms.extensionality. intro b0.
...
- (* Lreturn *)
  ...
  apply Mem.support_free in C as SF'.
  apply Mem.support_free in H as SF.
  assert (SEQ: Mem.stackseq m' m1'). unfold Mem.stackseq in *.
  congruence.
  exploit Mem.return_frame_parallel_stackseq; eauto.
  intros (m2' & RET & SEQ'').
  ...
  eapply Mem.sup_include_return_frame; eauto.
  rewrite globalenv_support in SEP.
  rewrite sep_swap; exact SEP. rewrite <- SF.
  eapply Mem.sup_include_return_frame; eauto.
...
- (* internal function *)
  ...
  unfold Genv.find_funct in FIND. destr_in FIND.
  eapply match_stacks_type_sp; eauto.
  eapply match_stacks_type_retaddr. eauto.
  intro. exploit H1; eauto.
  clear SEP;
  intros (j' & rs' & m1' & m2' & sp' & m3' & m4' & m5' & A & B & C & D & E & F &  G & SEP & J & K & L & M & N).
  ...
  erewrite (Mem.support_alloc _ _ _ _ _ H0); eauto.
...
Proof.
  ...
  - monadInv TR. econstructor; eauto. inv H5. apply Hg in H8. destr_in H8.
    eauto.
...
Proof.
  ...
  - cbn [stack_contents] in SEP. rewrite <- sep_assoc in SEP.
    ...
    destruct w; cbn in *.
    eapply cc_stacking_mr_intro with (w' := injw j _ _ SINJ); cbn; eauto.
    + destruct stk_w as [j0 nb1 nb2]; cbn in *. constructor; eauto.
...
Proof.
  ...
  eexists (stkw injp (injpw _ _ _ MINJ SINJ) _ ls (parent_sp cs') m'), _. repeat apply conj.
  - econstructor; eauto.
