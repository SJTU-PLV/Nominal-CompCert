...
- set (sg := RTL.funsig fd) in *.
  ...
  exploit (exec_moves mv1); eauto. intros [ls1 [A1 B1]].
  exploit ros_address_lessdef; eauto.
  eapply add_equations_args_satisf; eauto.
  intros RLD.
  ...
  subst vf. rewrite H0 in RLD. inv RLD. eauto.
  eauto. traceEq.
...
- set (sg := RTL.funsig fd) in *.
  ...
  exploit ros_address_lessdef; eauto. eapply add_equations_args_satisf; eauto.
  intros RLD.
...
Proof.
  ...
  destruct Hq1i. CKLR.uncklr.
  inversion Hqi2; clear Hqi2. subst sg. subst. inv Hst1.
  assert (vf2 = Vptr (Global id) Ptrofs.zero). inv H. eauto.
  ...
  exists (LTL.Callstate (Stackbase rs0 :: nil) vf2 rs0 m2 id).
