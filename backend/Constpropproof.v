...
Inductive match_states: nat -> state -> state -> Prop :=
  ...
  | match_states_call:
      forall s vf args m s' vf' args' m' id
           (STACKS: list_forall2 match_stackframes s s')
...
- (* Icall *)
  ...
  TransfInstr; intro.
  left; econstructor; econstructor; split.
  eapply exec_Icall; eauto. subst vf. rewrite H0 in FEXT.
  inv FEXT.
  cbn. eauto.
...
- (* Itailcall *)
  ...
  subst vf. rewrite H0 in FEXT. inv FEXT. cbn. eauto.
  apply sig_function_translated; auto.
  constructor; auto.
  apply regs_lessdef_regs; auto.
...
Proof.
  intros. destruct H. CKLR.uncklr. inv H0.
  exploit functions_translated; eauto. intros FIND.
  inversion H. subst.
  exists O; exists (Callstate nil (Vptr (Global id) Ptrofs.zero ) vargs2 m2 id); split.
